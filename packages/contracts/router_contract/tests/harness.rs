...
async fn exchange_contract() {
    ...
    let router_instance = RouterBuilder::new(router_contract_id.to_string(), wallet.clone()).build();
    ...
    let response = router_instance
        ...
        .await
        .unwrap();
    assert_eq!(response.value.output_amount, amount_expected.value.amount);
    ...
    let response = router_instance
        ...
        .await
        .unwrap();
    assert_eq!(response.value.input_amount, amount_expected.value.amount);
